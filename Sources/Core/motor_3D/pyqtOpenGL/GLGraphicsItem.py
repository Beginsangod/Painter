...
class PickColorManager(dict):
    ...
    def new_item(self, item) -> np.float32:
        """
        Assign a 32-bit floating-point color in the range [0, 1] to each item for color-based picking
    ...
    def del_item(self, item: 'GLGraphicsItem') -> None:
        """
        Lors de la suppression d’un élément, supprimer également la couleur correspondante afin d’éviter les fuites de mémoire
...
class GLGraphicsItem(QtCore.QObject):
    ...
    def __init__(
        ...
        # Attribuer une couleur lors du picking, utilisée pour l’identification des couleurs pendant le picking, afin de permettre la sélection d’un objet.
