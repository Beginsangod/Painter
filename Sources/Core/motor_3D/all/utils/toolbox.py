...
class ButtonItem(QPushButton, ToolItem):
    ...
    def __init__(self, label="Bouton", value=False, checkable=False, callback: Callable=None):
...
class ArrayTypeItem(QtWidgets.QWidget, ToolItem):
    ...
    def __init__(self, label:str, value:NumberTuple,
        ...
        self.type = type
        assert self.length > 0, "Array doit être supérieur à 0"
...
class DragArrayItem(QtWidgets.QWidget, ToolItem):
    ...
    def _validate_arg(self, arg) -> List[Number]:
        if isinstance(arg, (list, tuple, np.ndarray)):
            assert len(arg) == self.length, "arg length doit être égal à length"
            return arg
        elif isinstance(arg, (int, float)) or arg is None:
            return [arg] * self.length
        else:
            raise TypeError(f"arg doit être une liste,  un tuple, un int ou un float, mais vous avez {type(arg)}")
...
class ToolBox():
    ...
    def _add_item(cls, label:str, item: ToolItem):
        ...
        if label in cls.Items.keys():
            raise KeyError("Ce label existe déjà")
