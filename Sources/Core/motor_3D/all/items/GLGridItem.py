...
def make_grid_data(size, spacing):
    """
    Génère les coordonnées des lignes pour une grille XY.

    :param size: tuple (x, y) définissant la taille de la grille
    :param spacing: tuple (dx, dy) définissant l'espacement entre les lignes
    :return: tableau Nx3 des positions des lignes
    """
    x, y = size
    dx, dy = spacing
    ...
    # Ajouter une colonne de z=0 pour passer en 3D
...
class GLGridItem(GLGraphicsItem, LightMixin):
    """
    Affiche un plan XY avec une grille et une surface colorée.
    Hérite de LightMixin pour supporter l'éclairage par des PointLights.
    """
    def __init__(
        self,
        size=(1., 1.),                  # Taille du plan
        spacing=(1., 1.),               # Espacement des lignes
        color=(1., 1., 1., 1),          # Couleur RGBA de la surface
        ...
        # Coordonnées des coins de la surface
        x, y = size
        ...
        self.rotate(90, 1, 0, 0)  # Rotation pour aligner le plan
        self.setDepthValue(-1)      # Profondeur pour que la grille soit derrière
        self.addLight(lights)       # Ajout des lumières
    ...
    def paint(self, model_matrix=Matrix4x4()):
        """
        Rendu de la surface et des lignes de la grille.
        """
        self.setupGLState()
