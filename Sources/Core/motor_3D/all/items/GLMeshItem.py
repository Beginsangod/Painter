...
class GLMeshItem(GLGraphicsItem, LightMixin):
    """
    Affiche un maillage 3D avec gestion de lumière.
    Supporte les normales calculées, coordonnées de texture et matériaux.
    """
    ...
    def __init__(
        ...
        if mesh is not None:
            # Utiliser un Mesh déjà existant
            self._mesh = mesh
        else:
            # Créer un nouveau Mesh à partir des données fournies
            self._mesh = Mesh(
                vertexes, indices, texcoords, normals,
    ...
    def paint(self, model_matrix=Matrix4x4()):
        ...
        # Revenir au rendu plein
        # gl.glPolygonMode(gl.GL_FRONT_AND_BACK, gl.GL_FILL)

    def setMaterial(self, material):
        """Met à jour le matériau du mesh."""
