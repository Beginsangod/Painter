...
# ------------------------------------------------------
# Classe GLAxisItem : affiche les axes X, Y, Z
# ------------------------------------------------------
class GLAxisItem(GLGraphicsItem):
    """
    Affiche trois lignes indiquant l'origine et l'orientation
    du système de coordonnées local.
    """

    # Positions de départ des axes (origine)
    stPos = np.array([
        0.0, 0.0, 0.0,  # axe X
        0.0, 0.0, 0.0,  # axe Y
        0.0, 0.0, 0.0,  # axe Z
    ...
    # Couleurs des axes (rouge, vert, bleu)
    colors = np.array([
        1.0, 0.0, 0.0,  # X rouge
        0.0, 1.0, 0.0,  # Y vert
        0.0, 0.0, 1.0,  # Z bleu
    ...
    def __init__(
        self,
        size=Vector3(1.,1.,1.),  # taille des axes
        width=2,                  # largeur des lignes
        tip_size=1,               # taille des pointes
        antialias=True,           # lissage des lignes
        glOptions='opaque',       # options OpenGL
        fix_to_corner=False,      # fixer les axes dans un coin de la vue
        parentItem=None
    ):
        ...
        if fix_to_corner:
            # permet de toujours dessiner les axes au-dessus des autres objets
            self.updateGLOptions({"glClear": (gl.GL_DEPTH_BUFFER_BIT,)})
            self.setDepthValue(1000)  # garantit qu'ils sont dessinés en dernier

        self.antialias = antialias

        # création de la géométrie des pointes de flèches
    ...
    def setSize(self, x=None, y=None, z=None):
        """
        Définit la taille des axes (dans le système de coordonnées local)
        :param x: longueur de l'axe X
        :param y: longueur de l'axe Y
        :param z: longueur de l'axe Z
        """
    ...
    def initializeGL(self):
        """
        Initialise les shaders et buffers OpenGL pour les axes et les pointes
        """
        # shader pour les lignes
        self.shader = Shader(vertex_shader, fragment_shader, geometry_shader)
        # shader pour les cones (pointe)
        self.shader_cone = Shader(vertex_shader_cone, fragment_shader)

        # VAO et VBO pour les lignes
        self.vao_line = VAO()
        self.vbo1 = VBO(
        ...
        # VAO et VBO pour les pointes (cones)
        ...
        # couleur instanciée pour les cones
        self.vbo1.bind()
        self.vbo1.setAttrPointer(2, divisor=1, attr_id=5)

        # EBO pour les triangles de cones
    ...
    def paint(self, model_matrix=Matrix4x4()):
        """
        Dessine les axes et les pointes
    ...
    # ------------------------------------------
    # Méthode projection-vue spéciale
    # ------------------------------------------
    def proj_view_matrix(self):
        """
        Si les axes sont fixés dans un coin, calcule une projection adaptée
        pour qu'ils apparaissent toujours au premier plan
        """
        if self.__fix_to_corner:
            ...
            # calcul de la position dans le coin supérieur droit
...
# ------------------------------------------
# Geometry Shader pour les lignes
# ------------------------------------------
geometry_shader = """
#version 330 core
layout(points) in;
