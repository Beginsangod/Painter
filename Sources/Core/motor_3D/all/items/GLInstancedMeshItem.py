...
from .MeshData import vertex_normal
...
class GLInstancedMeshItem(GLGraphicsItem, LightMixin):
    """
    Affiche un maillage instancié (comme des cônes répétés) avec couleurs et lumière.
    Supporte la mise à jour dynamique des positions et couleurs des instances.
    """

    def __init__(
        self,
        pos=None,  # nx3 positions des instances
        vertexes=None,  # vertices du maillage
        indices=None,   # indices pour glDrawElements
        normals=None,   # normales
        lights=list(),
        color=[1., 1., 1.],
        size=1.,
        opacity=1.,
        ...
        # Calcul des normales si nécessaire
    ...
    def setData(self, pos=None, color=None, size=None, opacity=None):
        """Met à jour positions, couleurs, taille et opacité."""
        ...
        if pos is not None:
            ...
            self._gl_update_flag = True
            # Étend la couleur si nécessaire pour correspondre aux instances
            if self._color is not None and self._color.size != self._pos.shape[0] * 3:
    ...
    def initializeGL(self):
        """Initialise VAO, VBO, EBO et shader."""
        self.shader = Shader(vertex_shader, fragment_shader)
        self.vao = VAO()

        # Maillage (vertices + normales)
        self.vbo_mesh = VBO([self._vertices, self._normals], [3, 3], gl.GL_STATIC_DRAW)
        self.vbo_mesh.setAttrPointer([0, 1], divisor=0, attr_id=[0, 1] if self._calcNormals else [0])

        if self._indices is not None:
            self.ebo = EBO(self._indices)

        # Positions et couleurs des instances
        self.vbo_pos = VBO([self._pos, self._color], [3, 3], usage=gl.GL_DYNAMIC_DRAW)
        self.vbo_pos.setAttrPointer([0, 1], [2, 3], divisor=1)

    def updateGL(self):
        """Met à jour les positions et couleurs sur le GPU."""
...
void main() {
    ...
    if (nr_point_lights == 0) result = oColor;
